...
// Start Match
void startMatch(long time) {
    stepper.moveTo(POSITION_UP);
    for (int i = 0; i < NUM_LEDS_POLLER_STATUS; i++) {
        leds_poller[i] = CRGB(200, 90, 0);
        FastLED.show();
        delay(10);
    }
    ...
    for (int i = 0; i < 3; i++) {
        for (int num = 0; num < NUM_LEDS_POLLER_STATUS; num++) {
            leds_poller[num] = CRGB::Red;
        }
        FastLED.show();
        delay(1000);
        for (int num = 0; num < NUM_LEDS_POLLER_STATUS; num++) {
            leds_poller[num] = CRGB::Black;
        }
        FastLED.show();
        delay(1000);
    }
    for (int num = 0; num < NUM_LEDS_POLLER_STATUS; num++) {
        leds_poller[num] = CRGB::Green;
    }
    FastLED.show();
}
